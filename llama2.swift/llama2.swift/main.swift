...
////        o[j] = weight[j] * (ss * x[j])
////    }
//    let o_t = UnsafeMutablePointer<Float>.allocate(capacity: o.count)
//    for i in 0..<o.count {
//       o_t[i] = o[i]
//    }
//
//    let x_t = UnsafeMutablePointer<Float>.allocate(capacity: x.count)
//     for i in 0..<x.count {
//          x_t[i] = x[i]
//     }
//
//     let weight_t = UnsafeMutablePointer<Float>.allocate(capacity: weight.count)
//        for i in 0..<weight.count {
//            weight_t[i] = weight[i]
//        }
//
//     rmsnormTest(o_t, x_t, weight_t, Int32(size))
//
//     for i in 0..<o.count {
//          o[i] = o_t[i]
//     }
...
//
//        
//        // 创建一个新的xout数组。并且把它转化成float*用于传递给matmulTest
//
//        // 使用UnsafeMutablePointer创建一个新的xout数组
//     let xout_t = UnsafeMutablePointer<Float>.allocate(capacity: xout.count)
//        for i in 0..<xout.count {
//            xout_t[i] = xout[i]
//        }
//     let x_t = UnsafeMutablePointer<Float>.allocate(capacity: x.count)
//        for i in 0..<x.count {
//            x_t[i] = x[i]
//        }
//     let w_t = UnsafeMutablePointer<Float>.allocate(capacity: w.count)
//        for i in 0..<w.count {
//            w_t[i] = w[i]
//        }
//     matmulTest(xout_t, x_t, w_t, Int32(n), Int32(d))
//    
//    for i in 0..<xout.count {
//        xout[i] = xout_t[i]
//    }
...
//    let statePtr = UnsafeMutablePointer<UInt64>.allocate(capacity: 1)
//    statePtr.initialize(to: state)
//    let res = random_u32(statePtr)
//    
//    return res
//}
//
...
//    let floatPointer = UnsafeMutablePointer<Float>.allocate(capacity: logits.count)
//    floatPointer.initialize(from: &logits, count: logits.count)
//
//    let next = runSample(Int32(sampler.vocab_size), sampler.temperature,sampler.topp, sampler.rng_state, floatPointer);
//    
//    return Int(next)
//    
////    var next: Int
